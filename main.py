    ...
    def _run() -> None:
        ...
        try:
            if req.action == DownloadAction.BEST:
                filepath = download_best(req.url, task_id, cb)
            elif req.action == DownloadAction.BEST_MP4:
                filepath = download_best_mp4(req.url, task_id, cb)
            elif req.action == DownloadAction.AUDIO_ONLY:
                filepath = download_audio_only(req.url, task_id, cb)
                ...
                if not req.format_id:
                    ...
                    return
                filepath = download_and_merge(req.url, req.format_id, task_id, cb)
            else:
                # SELECTED
                if not req.format_id:
                    cb(DownloadProgress(
                    ...
                    return
                filepath = download_format(req.url, req.format_id, task_id, cb)
